<template>
  <div ref="anchor" class="anchor-wrapper" :style="{ top: anchorPosition.top, left: anchorPosition.left}">
    <anchor-tree  class="tree" :title-list="titleList" @locateTitle="handleLocateTitle"></anchor-tree>
    <!-- <header></header> -->
...
export default {
  ...
  watch:{
    list(newVal){
      ...
      this.handleLocateTitle(newVal)
    },
    activeAnchorTop(n){
      console.log('kksksj',this.currentAnchorId)
      this.$emit('active',this.currentAnchorId)
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    computedAnchorPosition() {
      ...
      // console.log('position',this.anchorPosition.top)
    },
    // 获取滚动内容的外层容器对象及DOM
    getScrollObj() {
      if(this.container){
        ...
        console.log('this.scrollEle',this.scrollEle);
      } else {
    ...
    handleLocateTitle(data) {
      ...
      console.log('this.scrollEle',this.scrollEle.scrollTop);
      // console
      setTimeout(() => {
    ...
    setActiveAnchorCirclePosition() {
      this.$nextTick(() => {
        ...
        if(currentAnchorEl){
          ...
          console.log('ss',this.activeAnchorTop)
    
        console.log('this.bb',this.activeAnchorTop);
