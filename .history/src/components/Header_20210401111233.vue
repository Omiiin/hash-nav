<template>
    <header class="header" :style="{backgroundColor: bg_color, position: hd_position}">
        <div>
            <div :class="{img_hover: hover_flag}" class="web_icon">
                <img src="../assets/bg_logo.png" alt="官网" @click="toHome">
            </div>
            <ul>
                <li v-for="(item,index) in titleList" :key="index" @click="selected(item.label)">
                    <router-link :to="item.link" :class="{active: active == item.label}">{{ item.label }}</router-link>
                </li>
            </ul>
        </div>
    </header>
