  ...
  methods: {
    ...
    getAllTitleOffsetTop() {
      ...
      while (++i < len) {
        this.allTitleOffsetTop.push(
          document.querySelector(`#${this.titleList[i].id}`).offsetTop -
            document.querySelector("#mainBox").offsetTop
        );
      }
    },
    // 根据内容滚动距离获取当前锚点数据
    getCurrentAnchor(scrollTop) {
      ...
      while (++i < len) {
        ...
        console.log("curAnchorScrollTop", curAnchorScrollTop);
        if (
          scrollTop >= curAnchorScrollTop &&
          nextAnchorScrollTop &&
          scrollTop < nextAnchorScrollTop
        ) {
          this.eId = this.titleList[i].id;
          break;
        }
      }
    },
